...
// Python.
namespace MITRE.QSD.QSharpReference {
    ...
    operation SingleQubitGates () : Unit {
        ...
        // Note that you set the scope of a use statement explicitly with
        // curly braces:
        // use qubit = Qubit() {
        //     *perform ops involving qubit*
        // }
        // *qubit is now deallocated*
    ...
    operation MultiQubitGates () : Unit {
        ...
        // Applies the H gate to each of the elements in qubits
        ApplyToEach(H, qubits); 

        // Similar to ApplyToEach(H, qubits), but the modifier A indicates
        // that the single-qubit operation is adjointable. The modifier
        // can be changed to C (ApplyToEachC(H, qubits)) to indicate that
        // the single-qubit operation is controllable or to CA
        // (ApplyToEachCA(H, qubits)) to indicate that the operation is
        // both controllable and adjointable. 
        ApplyToEachA(H, qubits);

        // Measures all the qubits in one utility functions
        let results = MultiM(qubits); 

        // Converts the array with type Results to an array with type Bool
        ...
        // Constructs a BigEndian type register from an array of qubits
        let bigE = BigEndian(qubits);

        // The ! is used to unwrap the BigEndian or LittleEndian type
        // register, so that each element of the register can be accessed
        // as a Qubit type
        for i in 0 .. Length(bigE!) - 1 {
            H(register![i]); 
        }

        // Constructs a LittleEndian type register from an array of qubits
        let littleE = LittleEndian(qubits);

        // Converts a BigEndian type to LittleEndian type register
        let bigEtoLittleE = BigEndianAsLittleEndian(bigE);
    }
    ...
    /// # Summary
    /// The within-apply syntax is a convenient way to reverse the setup for
    /// some operation.
    operation WithinApply () : Unit {
        use (controls, target) = (Qubit[1], Qubit());
